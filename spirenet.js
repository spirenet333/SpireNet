...
const DB_NAME = "spirenet_cli_db_v14";
const DB_VERSION = 1;
...
function resolvePath(fromDirId, raw) {
  ...
  for (const part of parts) {
    if (part === ".") continue;
/* =========================
   business helpers + column formatting
========================= */
...
function formatListIntoColumns(items, rowsPerColumn = 10) {
  ...
  const colWidth = Math.max(16, Math.min(longest + 4, 40));

  const usableWidth = Math.max(200, terminal.clientWidth - 24);
  const colsPerPage = Math.max(1, Math.floor(usableWidth / colWidth));
  const itemsPerPage = rowsPerColumn * colsPerPage;

  const lines = [];

  for (let pageStart = 0; pageStart < items.length; pageStart += itemsPerPage) {
    for (let row = 0; row < rowsPerColumn; row++) {
      let line = "";

      for (let col = 0; col < colsPerPage; col++) {
        const index = pageStart + (col * rowsPerColumn) + row;
    ...
    if (pageStart + itemsPerPage < items.length) {
      lines.push("");
    }
  }
